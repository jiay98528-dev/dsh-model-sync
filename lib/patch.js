...
function configBooleanValue(lines, block, key) {
    const configIndent = block.indent + 2;
    let configAt = -1;
    for (let index = block.start + 1; index < block.end; index++) {
        const match = /^(\s*)config:\s*(?:#.*)?$/.exec(lines[index]);
        if (match?.[1].length === configIndent) {
            configAt = index;
            break;
        }
    }
    ...
    for (let index = configAt + 1; index < block.end; index++) {
        ...
        if (match?.[1].length === valueIndent)
            return match[2] === 'true';
    }
    return undefined;
}
function featureEnabled(text, id, key, requiredIndent) {
    const lines = text.split(/\r?\n/);
    const block = findEntryBlock(lines, id, requiredIndent);
...
function updateLoaderOverride(text, id, enabled) {
    const lines = text.split(/\r?\n/);
...
function updateFeatureOverride(text, id, key, enabled) {
    const lines = text.split(/\r?\n/);
    const block = findEntryBlock(lines, id, 0);
    if (block) {
        const configIndent = block.indent + 2;
        ...
        if (configAt < 0) {
            lines.splice(block.start + 1, 0, `${' '.repeat(configIndent)}config:`, `${' '.repeat(configIndent + 2)}${key}: ${enabled}`);
        }
        else {
            let valueAt = -1;
            for (let index = configAt + 1; index < block.end; index++) {
            ...
            if (valueAt >= 0)
                ...
                lines.splice(configAt + 1, 0, line);
        }
    }
    else {
...
function updateEntryEnabled(profile, id, enabled) {
    ...
    if (!plugin || bundleEntryEnabled(profile, plugin.entryId, plugin.packageName) === undefined)
        return false;
    const path = profilePatchPath(profile);
    const text = existsSync(path) ? readFileSync(path, 'utf8') : '[]\n';
