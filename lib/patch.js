...
function configEnabledValue(lines, block) {
    ...
    for (let index = configAt + 1; index < block.end; index++) {
        ...
        if (match?.[1].length === enabledIndent)
            return match[2] === 'true';
    }
    return undefined;
}
function featureEnabled(text, id, requiredIndent) {
    const lines = text.split(/\r?\n/);
    const block = findEntryBlock(lines, id, requiredIndent);
...
function updateFeatureOverride(text, id, enabled) {
    const lines = text.split(/\r?\n/);
    const block = findEntryBlock(lines, id, 0);
    if (block) {
        const configIndent = block.indent + 2;
        ...
        if (configAt < 0) {
            lines.splice(block.start + 1, 0, `${' '.repeat(configIndent)}config:`, `${' '.repeat(configIndent + 2)}enabled: ${enabled}`);
        }
        else {
            let enabledAt = -1;
            for (let index = configAt + 1; index < block.end; index++) {
