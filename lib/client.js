	...
	factory: (require) => {
		...
		var I18N = {
			zh: {
				...
				refreshing: '刷新中',
				pluginsToggle: '功能与插件',
				pluginsHint: '模型同步控制模型发现与写入；可选插件使用 disabled 标志',
				usageToggle: '模型用量查询',
			...
			en: {
				...
				pluginsHint: 'Model Sync controls model discovery and writes; optional plugins use the disabled flag.',
			...
			return h('span', { className: 'msync-wrap', tabIndex: 0, title: title },
				h('span', { className: 'msync-ring', 'aria-label': t('quota') },
					h('svg', { width: size, height: size, viewBox: '0 0 ' + size + ' ' + size },
						winCount === 2
							? [
								h('circle', { key: 't7', cx: cx, cy: cx, r: rOuter, fill: 'none', stroke: 'var(--dsw-alias-border-l2)', strokeWidth: strokeOuter }),
								h('circle', {
									key: 'a7', cx: cx, cy: cx, r: rOuter, fill: 'none',
									stroke: quotaColor(p7),
									strokeWidth: strokeOuter, strokeLinecap: 'round',
									strokeDasharray: String(cOuter),
									strokeDashoffset: String(cOuter * (1 - p7)),
									transform: 'rotate(-90 ' + cx + ' ' + cx + ')',
								}),
								h('circle', { key: 't5', cx: cx, cy: cx, r: rInner, fill: 'none', stroke: 'var(--dsw-alias-border-l2)', strokeWidth: strokeInner }),
								h('circle', {
									key: 'a5', cx: cx, cy: cx, r: rInner, fill: 'none',
									stroke: quotaColor(p5),
									strokeWidth: strokeInner, strokeLinecap: 'round',
									strokeDasharray: String(cInner),
									strokeDashoffset: String(cInner * (1 - p5)),
									transform: 'rotate(-90 ' + cx + ' ' + cx + ')',
								}),
							]
							: [
		...
		function SettingsPage(props) {
			...
			return h('div', { className: 'msync-page' },
				...
				h('details', { className: 'msync-details' },
					h('summary', null, t('pluginsToggle')),
					h('div', { className: 'body' },
						h('div', { className: 'hint' }, t('pluginsHint')),
						(state && state.plugins ? state.plugins : []).filter(function (plugin) {
							return plugin.id !== 'model-sync-usage';
						}).map(function (plugin) {
			...
			return h('div', { className: 'msync-page' },
				h('div', { className: 'msync-row' },
					...
					h('div', { className: 'msync-row', style: { gap: '8px' } },
						h('label', { className: 'msync-row', style: { gap: '8px' } },
							h('span', { className: 'name' }, t('usageToggle')),
							h('input', {
								className: 'msync-switch',
								type: 'checkbox',
								checked: usageActive,
