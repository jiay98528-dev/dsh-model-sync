window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var CSS_ID = '@agentteam/model-sync/ui.css';
		var CSS = [
		...
		function ensureCss() {
			...
			tag.dataset.plugin = '@agentteam/model-sync';
		...
		function DualRing(props) {
			...
			var c5 = 2 * Math.PI * 6;
			var c7 = 2 * Math.PI * 10;
			return h('span', { className: 'msync-wrap', tabIndex: 0, title: '额度' },
					...
					providers.length === 0
						...
						: providers.map(function (prov) {
							...
							var text = rows.length
								? rows.map(function (w) { return w.label + ' 剩余 ' + Math.round(pct(w) * 100) + '%'; }).join(' · ')
								: (q.reason || '无数据');
							return h('div', { key: prov.id, className: 'line' }, h('span', null, '内 5h / 外 7d'), h('span', null, text));
						}),
				),
			);
		}
