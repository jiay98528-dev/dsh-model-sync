...
1. **双层用量环语义交替**：此前外环展示 7 天窗口、内环展示 5 小时窗口，现改为 **外环 = 5h、内环 = 7d**（与屏幕阅读顺序、视线焦点一致）。仅修改 `DualRing` 组件的 `strokeDashoffset` 绑定与 React `key`（避免 DOM 复用错位），`win5h`/`win7d` 字典键与 detail panel 行序不动。
2. **i18n 同步**：`innerOuter` 文案改为「内 7d / 外 5h」/「Inner 7d / outer 5h」（中英双语）；悬浮提示与点击面板里的窗口行顺序保持原意。
